...
def main():
    ...
    processed_ids = [pid for pid in processed_ids if pid]  # 过滤空ID
    if processed_ids:
        history.save_papers(processed_ids)
        print(f"  已保存 {len(processed_ids)} 篇论文到历史记录")

    # Step 5: 生成简短摘要（所有相关论文）
    print("\n[5/6] 生成摘要...")
    ...
    # Step 5.1: Top5 详细摘要
    top5_papers = sorted_papers[:5]
    other_papers = sorted_papers[5:]
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_map = {
            executor.submit(_detailed_task, item): item for item in top5_papers
        }
        completed = 0
        for future in as_completed(future_map):
            completed += 1
            ...
            paper = item["paper"]
            try:
                top5_detailed.append(future.result())
            except Exception as e:
            ...
            print(f"  [Top完成] {completed}/{len(top5_papers)}")
