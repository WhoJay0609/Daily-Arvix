...
- 🏆 **Top5 详细摘要**：对评分最高的 5 篇论文生成详细分析（背景、动机、方法）
- 📝 **简洁概述**：其他相关论文生成简短摘要
- 📅 **每日归档**：自动保存每天的日报历史
...
| `MAIL_TO` | 收件人 | `recipient@example.com` |
...
3. **相关性判断**：LLM 评估每篇论文与兴趣方向的相关性并打分
4. **过滤排序**：保留相关论文并按评分排序
5. **生成摘要**：
   - Top5：生成详细摘要（背景/动机/方法/总结）
   - 其他：生成简短摘要（1-2 句）
6. **保存归档**：生成 Markdown 日报并归档
7. **历史记录**：保存已处理论文 ID，下次运行时跳过
8. **邮件发送/企业微信推送**：通过 SMTP 或企业微信 Webhook 发送
